...
int API_draw_text(int x, int y, uint8_t color, char *str_in, char *fontname, int frontsize, int fontstype, int reserved)
{
	//frontsize <-- moet nog
	//fontstype <-- moet nog
	//formule voor bedenken.

	int array_teller = fontstype;	//0 is dik, 1 is schuin, 2 is normaal.
	  ...
	  for(pstr = strtok (str_buf," ");pstr!=NULL;pstr = strtok (NULL, " "))
		{
		  ...
		  {
			Draw_Letter(x,y,word_buf[tel],color,array_teller);	//a,b,c,
			...
			{
			  Draw_Letter(x,y,' ',color,array_teller);
			  x+=16;
			}
		}
	}
